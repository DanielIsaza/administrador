...
@section('content')
		<script>
			...
			$(document).ready(function()
			{
				...
				{ espacio:$(this).val() },
				function(data){		
					$("select[name=idGrupo").empty();
					$("select[name=idGrupo]").append("<option >"+"Seleccione una opción"+"</option>")
					$.each(data,function(key,element)
					{
						$("select[name=idGrupo").append("<option value='"+key+"'>"+element+"</option>");
					});
				});
			});
		});
	</script>
	{!! Form::open(['route' => 'reporteGrupo.store','method' => 'POST']) !!}
		...
		<div class="form-group">
			...
			{!! Form::select('idGrupo',[""],null,['class'=> 'form-control','placeholder'=>'Seleccione una opción','required']) !!}
		</div>
		<div class="form-group">
